...
(UUID(), 'Merchant', 'merchant_id', 'multiselect', 'SELECT id, name FROM merchants WHERE active = TRUE LIMIT 500', TRUE),
(UUID(), 'Tier', 'tier', 'select', 'SELECT DISTINCT tier FROM merchants WHERE tier IS NOT NULL', TRUE),
(UUID(), 'Account Manager', 'am', 'select', 'SELECT DISTINCT account_manager FROM merchants WHERE account_manager IS NOT NULL', TRUE),
(UUID(), 'Region', 'region', 'select', 'SELECT DISTINCT region FROM addresses WHERE region IS NOT NULL', TRUE);
...
FROM deliveries d
LEFT JOIN addresses a ON d.delivery_address_id = a.id
